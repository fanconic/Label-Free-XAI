...
class ProtoAutoEncoderMnist(nn.Module):
    ...
    def proto_loss(self, min_distances: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        if self.metric == "cosine":
            comb = torch.cartesian_prod(
                torch.arange(0, self.n_prototypes),
                torch.arange(0, self.n_prototypes),
            )

            # Reshape the vector to [B, 32]
            vectors_reshaped = self.protolayer.view(self.protolayer.size(0), -1)

            # Calculate pairwise cosine similarity
            proto_sim = torch.nn.functional.cosine_similarity(
                vectors_reshaped.unsqueeze(1), vectors_reshaped.unsqueeze(0), dim=2
            )

            # Fill the diagonal with zeros (cosine similarity with itself is always 1)
            proto_sim.fill_diagonal_(0)
            proto_sim += torch.diag(-3 * torch.ones(self.n_prototypes))
            proto_min_sim, _ = torch.max(proto_sim, dim=1)
            diversity_loss = torch.mean(proto_min_sim)

        elif self.metric == "l2":
            proto_dist = torch.cdist(
                self.protolayer.squeeze(-1).squeeze(-1),
                self.protolayer.squeeze(-1).squeeze(-1),
                p=2,
            ) / np.sqrt(self.latent_dim)
            proto_dist += torch.diag(200 * torch.ones(self.n_prototypes))
            proto_min_dist, _ = torch.min(proto_dist, dim=1)
            assert torch.all(proto_min_dist < 200)
            diversity_loss = -torch.mean(proto_min_dist)

        # Make sure that the distance from the sample to the closest is not large
        min_distance, _ = torch.min(min_distances, dim=1)
        cluster_loss = torch.mean(min_distance)

        return diversity_loss, cluster_loss
    ...
    def loss(
        ...
        diversity_loss, cluster_loss = 0, 0  # self.proto_loss(min_distances)
        return (
            lambda_0 * base_loss + lambda_1 * diversity_loss + lambda_2 * cluster_loss
        )
