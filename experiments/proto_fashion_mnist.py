...
def proto_consistency_feature_importance(
    ...
    for run in range(runs):
        ...
        load_dir = Path.cwd() / f"results/fashion_mnist/predictive_performance/{pae_model_name}"
        protoautoencoder.eval()
        protoautoencoder.load_state_dict(
...
def proto_consistency_examples(
    ...
    for run in range(runs):
        ...
        sim_most, sim_least = proto_similarity_rates(
            attribution, labels_subtest, n_top_list, task="denoising", n_prototypes=n_prototypes, run=run, dataset="fashion_mnist"
        )
        results_list += [
    ...
    save_dir = Path.cwd() / "results/fashion_mnist/consistency_examples"
    if not save_dir.exists():
        os.makedirs(save_dir)
    ...
    plt.savefig(save_dir / "fashion_similarity_rates.pdf")
